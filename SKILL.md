...
- 最终面向用户输出必须是书面表达级别的交易结论；不要展示源码、命令参数、JSON 路径、脚本字段名、布尔值、空数组、`null`、内部状态或处理过程。
...
- 最终回复：依据 `score.verdict`、`score.composite`、`score.trade_plan`、`score.account_overlay`、`score.llm_overlay` 的取值组织自然语言结论；不要把这些字段名写给用户。
...
字段名只供内部定位取数。最终答复要把字段含义翻译成自然语言，例如把空风险项写成“未触发大盘风控”，把空数据风险写成“未发现数据质量问题”，把确认布尔值写成“技术确认不足/已确认”。除非用户明确要求原始 JSON 或调试信息，不要输出 `score.xxx`、`risk_gates=[]`、`data_flags=[]`、`confirmation.ok=false`、`trade_plan.status=...` 这类源码式表达。
...
- 所有价格、仓位、止损、止盈和封顶结论都必须来自脚本评分结果；缺字段时写“无法确认”，不要猜。
- 不要用 Markdown 代码样式包裹普通数值、状态或字段名；股票代码、命令和用户明确要求的原始字段例外。
- 不要把字段路径、snake_case 标识符、数组/对象字面量、命令参数或中间 artifact 写入最终回复。把它们改写为用户能直接理解的中文短语。
...
- 建议入场价: 写脚本给出的计划价；若不建议新开仓写“不建议入场”
- 建议出场价: 写脚本给出的止损价；若已有持仓优先写保护性出场价
- 止盈参考: 写第一/第二止盈价，并注明移动止损比例
- 强制排除条件: 写自然语言原因；没有写“无”
- 一句话: 最关键依据
...
| 过滤条件 | 状态 | 关键证据 | 处理建议 |
|---|---|---|---|
| 大盘 regime | 通过 / 风险 | 大盘趋势和是否触发风险；无风险写“未触发大盘风控” | 大盘风险时不新增或降至观察 |
| 个股趋势闸门 | 通过 / 风险 | MA60、MA200、周线排列和关键趋势位 | 跌破关键均线时不新开；已持仓按保护价管理 |
| 相对强度 | 通过 / 偏弱 / 无法确认 | 相对 SPY/QQQ 的 3m/6m 强弱 | 跑输基准则等待重评 |
| 30 日结构与追价 | 通过 / 过热 / 破位 | 30 日位置、回踩价、突破价、追价上限 | 高于追价上限不追；等回踩或突破确认 |
| 技术与量价确认 | 通过 / 未确认 | 技术确认、MACD/RSI/KDJ 和成交量是否达标 | 技术确认不足时只观察不买 |
| 流动性/数据质量 | 通过 / 风险 / 无法确认 | 历史数据是否足够、snapshot 新鲜度、价差和成交量；无风险写“未发现数据质量问题” | 数据异常时补数据后重评 |
| 新闻/事件红旗 | 未触发 / 低 / 中 / 高 / 无法确认 | 新闻红旗等级、可验证来源和是否触发封顶；无红旗写“未发现中高等级红旗” | 中高风险只降级；不可用时不用于升级 |
| 账户敞口 | 无持仓 / 已持仓 / 无法确认 / 风险 | 当前仓位、目标仓位、现金比例和是否超配 | 超目标减仓；无持仓按入场计划，不下单 |

**评分拆解**

读取评分拆解、确认状态、建议仓位和交易计划，但最终只写中文标签和数值。至少写明动量、相对强度、趋势效率三项的得分/贡献。若已封顶，说明触发的自然语言原因和封顶后的影响；不要输出字段名、路径或内部状态。

**账户敞口与交易计划**

固定放在“评分拆解”后、“新闻面风控”前。读取到 Alpaca 持仓时，写当前持仓市值占账户权益比例、脚本目标仓位、差额、成本价、浮盈亏、保护性出场价、移动止损、2R/3R 止盈参考、追价上限和账户处理建议。没有持仓时，写“当前无持仓”，仍要给出入场/止损/止盈/追价上限；账户读取失败时，写“账户持仓无法确认，未用于调整建议”。任何情况下都不要下单。

**新闻面风控**

固定放在“账户敞口与交易计划”后面。写新闻检索状态、Finnhub 补充状态、主要 catalyst、红旗、来源日期/链接、是否触发风控封顶。无红旗写“未发现可验证的重大新闻红旗，未影响评分”；新闻不可用写“新闻面无法确认，未用于降级”。Finnhub 新闻/财报只能帮助识别事件风险，不得把利好新闻用于加分。

**建议**

给出一个明确动作，并结合账户处理建议。若无持仓且交易计划允许入场，可以写“按计划价分批新开”；若已有持仓且 verdict 为 `持仓需减风险`，明确写“减仓至目标敞口 / 跌破保护性出场价退出”；若账户 overlay 不可用，动作只基于价量和新闻面。只有用户要求详细数据时，才追加 Alpaca 明细、完整 JSON 字段或 K 线图。
...
- 是否出现源码式或调试式文本，例如字段路径、snake_case 标识符、空数组/空对象、`true/false/null`、命令参数、artifact 名称或脚本内部状态；若出现，改写成自然语言。
