...
`portfolio.py` 通过 Alpaca CLI 只读调用 `account get` 与 `position list`，归一化账户权益、现金比例、总多头敞口、当前标的市值/成本/浮盈亏。`analysis.build_result()` 按 symbol 切出单票上下文传给 `score(..., account_context=...)`。
...
| `finnhub.py` | Finnhub quote/news/profile/earnings 可选补充上下文，过滤 token，结构化 HTTP 错误，并把明确负面新闻/临近财报压缩为 `llm_context` 候选 | 库 |
| `portfolio.py` | `alpaca account get` / `position list` 只读读取账户敞口和持仓，过滤账号标识 | 库 |
...
| `indicators.py` | CLI：拉数→分析→打分→输出 JSON，可用 `--llm-context-file` 传入新闻面风控 JSON，默认 `--account-context auto` 尝试读取持仓，`--finnhub-context auto` 在有 key 时读取补充上下文 | `python3 indicators.py` |
